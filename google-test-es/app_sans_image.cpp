...
static const unsigned num_images = 2;

static void* g_client_buffer[num_images];
	...
	const char* const * argv)
{
	...
	if (cli_err)
	{
		std::cerr << "app options (multiple args to an option must constitute a single string, eg. -foo \"a b c\"):\n"
			...
			" <width> <height>\t: use specified dimensions with source image\n" << std::endl;
	}
...
hook::deinit_resources()
{
	...
	for (unsigned i = 0; i < sizeof(g_client_buffer) / sizeof(g_client_buffer[0]); ++i)
	{
		free(g_client_buffer[i]);
		g_client_buffer[i] = 0;
	}
	...
	const char* const * argv)
{
	...
	{
		g_client_buffer[i] = malloc(g_image_w * g_image_h * 2);

		if (!g_client_buffer[i])
		{
			std::cerr << __FUNCTION__ << " failed at malloc" << std::endl;
			return false;
		}

		memset(g_client_buffer[i], 0x1f * i, g_image_w * g_image_h * 2);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, g_image_w, g_image_h, 0,
	...
	glBindVertexArray(g_vao[PROG_QUAD]);
...
hook::render_frame()
{
	...
	if (g_tex[idx] && -1 != g_uni[PROG_QUAD][UNI_SAMPLER_ALBEDO])
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, g_image_w, g_image_h, 0,
			GL_RGB, GL_UNSIGNED_SHORT_5_6_5, g_client_buffer[idx]);
