...
void main()
{
	vec4 p_lit_1 = p_lit_i + dFdx(p_lit_i) * .75;
	vec4 p_lit_2 = p_lit_i + dFdy(p_lit_i) * .75;

	vec3 shadow_z = vec3(
		texture2DProj(shadow_map, p_lit_i).x,
	...
	vec3 clip = step(p_z_lit, p_w_lit); // standard [-1, 1] ndc was biased to [0, 1]
	...
	vec3 lit = step(p_z_lit, shadow_z);

	float lit_average = dot(lit, vec3(1.0 / 3.0));
	d *= lit_average;
	s *= lit_average;
