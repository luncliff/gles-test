	...
	const char* const * argv)
{
	...
	for (unsigned i = 0; i < num_images; ++i)
	{
		const EGLint attr[] =
		{
			// EGL_FSL_create_image/EGL_QUERY_IMAGE_FSL combo appears rough around the edges -
			// there is no way to query image pitch. Make sure image width is properly aligned
			// for a texture target on the Z430, so no pitch alignment adjustments are needed.
			EGL_WIDTH, g_image_w,
...
hook::render_frame()
{
	...
	if (g_tex[idx] && -1 != g_uni[PROG_QUAD][UNI_SAMPLER_ALBEDO])
	{
		...
		static uint8_t grad;
		memset(g_client_buffer[idx].mem_virt[0], grad++, g_image_w * g_image_h * 2);
