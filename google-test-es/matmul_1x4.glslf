...
void main()
{
	...
	vec2 tcoord = gl_FragCoord.xy * tcoord_delta;
	vec4 a = texture2D(a_map, tcoord);

	float modulus = mod(gl_FragCoord.x, 4.0) * tcoord_delta.x;

	vec4 b0 = texture2D(b_map, vec2(tcoord.x - modulus + 0.5 * tcoord_delta.x, tcoord.y));
	vec4 b1 = texture2D(b_map, vec2(tcoord.x - modulus + 1.5 * tcoord_delta.x, tcoord.y));
	vec4 b2 = texture2D(b_map, vec2(tcoord.x - modulus + 2.5 * tcoord_delta.x, tcoord.y));
	vec4 b3 = texture2D(b_map, vec2(tcoord.x - modulus + 3.5 * tcoord_delta.x, tcoord.y));
