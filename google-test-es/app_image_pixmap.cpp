...
static Pixmap g_pixmap[num_images];
...
static void
updatePixmap(Pixmap pixmap)
{
	Display* display = reinterpret_cast< Display* >(util::getNativeWindowSystem());

	GC gc = DefaultGC(display, 0);

	static unsigned long foreground;
	XSetForeground(display, gc, foreground++);
	XFillRectangle(display, pixmap, gc, 0, 0, g_image_w, g_image_h);

	const char message[] = "pixmap to texture";
	XSetForeground(display, gc, 0x0000ff);
	XDrawString(display, pixmap, gc, 10, 15, message, strlen(message));
...
hook::deinit_resources()
{
	...
	for (unsigned i = 0; i < sizeof(g_image) / sizeof(g_image[0]); ++i)
	{
		...
		XFreePixmap(display, g_pixmap[i]);
		g_pixmap[i] = 0;
	}
	...
	const char* const * argv)
{
	...
	for (unsigned i = 0; i < num_images; ++i)
	{
		...
		g_pixmap[i] = XCreatePixmap(display, DefaultRootWindow(display),
			g_image_w, g_image_h, 16);
		...
		g_image[i] = eglCreateImageKHR(g_display, EGL_NO_CONTEXT, EGL_NATIVE_PIXMAP_KHR,
			(EGLClientBuffer) g_pixmap[i], attr);
