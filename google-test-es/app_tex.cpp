	...
	const char* const * argv)
{
	...
	g_shader_vert[PROG_TEX] = glCreateShader(GL_VERTEX_SHADER);
	assert(g_shader_vert[PROG_TEX]);

	if (!util::setupShader(g_shader_vert[PROG_TEX], "texture.glslv"))
	{
		std::cerr << __FUNCTION__ << " failed at setupShader" << std::endl;
		return false;
	}

	g_shader_frag[PROG_TEX] = glCreateShader(GL_FRAGMENT_SHADER);
	assert(g_shader_frag[PROG_TEX]);

	if (!util::setupShader(g_shader_frag[PROG_TEX], "texture.glslf"))
	{
		std::cerr << __FUNCTION__ << " failed at setupShader" << std::endl;
		return false;
	}

	g_shader_prog[PROG_TEX] = glCreateProgram();
	assert(g_shader_prog[PROG_TEX]);

	if (!util::setupProgram(
			g_shader_prog[PROG_TEX],
			g_shader_vert[PROG_TEX],
			g_shader_frag[PROG_TEX]))
	{
		std::cerr << __FUNCTION__ << " failed at setupProgram" << std::endl;
		return false;
	}

	g_uni[PROG_TEX][UNI_SAMPLER_ALBEDO] = glGetUniformLocation(g_shader_prog[PROG_TEX], "albedo_map");

	g_active_attr_semantics[PROG_TEX].registerVertexAttr(glGetAttribLocation(g_shader_prog[PROG_TEX], "at_Vertex"));
	g_active_attr_semantics[PROG_TEX].registerTCoordAttr(glGetAttribLocation(g_shader_prog[PROG_TEX], "at_MultiTexCoord0"));
	...
	glBindVertexArray(g_vao[PROG_TEX]);
	...
	if (!setupVertexAttrPointers< Vertex >(g_active_attr_semantics[PROG_TEX])
#if !defined(DEBUG)
		|| util::reportGLError()
#endif
		)
...
hook::render_frame()
{
	...
	glUseProgram(g_shader_prog[PROG_TEX]);

	DEBUG_GL_ERR()

	if (g_tex[TEX_ALBEDO] && -1 != g_uni[PROG_TEX][UNI_SAMPLER_ALBEDO])
	{
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, g_tex[TEX_ALBEDO]);

		glUniform1i(g_uni[PROG_TEX][UNI_SAMPLER_ALBEDO], 0);
	}

	DEBUG_GL_ERR()

	for (unsigned i = 0; i < g_active_attr_semantics[PROG_TEX].num_active_attr; ++i)
		glEnableVertexAttribArray(g_active_attr_semantics[PROG_TEX].active_attr[i]);
	...
	for (unsigned i = 0; i < g_active_attr_semantics[PROG_TEX].num_active_attr; ++i)
		glDisableVertexAttribArray(g_active_attr_semantics[PROG_TEX].active_attr[i]);
