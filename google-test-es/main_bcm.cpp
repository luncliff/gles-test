...
struct EGL
{
	...
	, window_h(h)
	{
		memset(&nativewindow, 0, sizeof(nativewindow));
	}
	...
	const bool print_configs)
{
	...
	surface = eglCreateWindowSurface(display, config[best_match], &nativewindow, 0);
	...
	char** argv)
{
	...
	{
		std::cerr << "unspecified framebuffer bitness; bailing out" << std::endl;
		return 1;
	}
