...
void main()
{
	...
	vec4 b0 = texture2D(b_map, tcoord - modulus + vec2(0.5, 0.5) * tcoord_delta);
	vec4 b1 = texture2D(b_map, tcoord - modulus + vec2(1.5, 0.5) * tcoord_delta);
	vec4 b2 = texture2D(b_map, tcoord - modulus + vec2(0.5, 1.5) * tcoord_delta);
	vec4 b3 = texture2D(b_map, tcoord - modulus + vec2(1.5, 1.5) * tcoord_delta);
